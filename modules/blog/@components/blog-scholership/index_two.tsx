...
const BlogStudyAbroad = ({ data, loading }: propTypes) => {
  const excludedIds: any = [];
  const dataStudyAbroad = data && data?.data.slice(0, 3);
  dataStudyAbroad?.map((item: any, i: any) => {
  ...
  const { data: studyAbroadLatestData } =
    useGetBlogByCategoryQuery(queryStringLatest);
  const len = dataStudyAbroad?.length;
  return (
    data && (
      <section className=" bg-[#F8F9FD] py-[60px]">
        <div className=" container ">
          <Link href="/blog/category/study-abroad">
            <h1 className=" text-h3-md mb-0 hover:underline">Study Abroad</h1>
          </Link>
          {loading ? (
                      ...
                      <BlogCardTwo
                        ...
                        classes={{
                          ...
                          imageWrapper: isFeature
                            ? "!mb-0 !h-[210px] lg:!h-[360px]"
                            : "!mb-0 lg:!h-[170px] !h-[150px]",
                          ...
                          title: `!line-clamp-2 leading-[1.3]    ${
                            isFeature
                              ? "lg:text-h2-sm text-c2 mt-2 "
                              : " lg:text-c2 text-c3 mt-[8px] !mb-0"
                          }`,
                          description: isFeature
                            ? " lg:text-p3 text-p4 line-clamp-2"
                            : "hidden",
                          date: isFeature
                            ? "text-p3"
                            : "lg:text-p5 text-[12px] mt-2 ",
...
export default BlogStudyAbroad;
