...
const UniversityOverviewDescription = ({ id, className, data }: propTypes) => {
  const [readmore, setReadmore] = useState(false);
  return (
    <>
      {data?.description ? (
        <section id={id} className={`pb-0 ${className ? className : ""}`}>
          <h2 className="h5 pb-4 mb-0">About {data?.name}</h2>
          <div
...
export default UniversityOverviewDescription;
